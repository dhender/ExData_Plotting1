...
# --------------------------------------------------------
# Alternatively if memory size is not a conern then read in entire file and create 
#txtfile <- read.table(dataFile,nrows = 2075259, sep=";", header=TRUE, quote= "", strip.white=TRUE, stringsAsFactors = FALSE, na.strings= "?")
# Convert the date variable to Date class
#txtfile$Date <- as.Date(txtfile$Date, format = '%d/%m/%Y')
#txtfile$DateTime <- as.POSIXct(paste(txtfile$Date, txtfile$Time))
# Subset the data
#txtfile <- subset(txtfile, subset = (Date >= "2007-02-01" & Date <= "2007-02-02"))
# --------------------------------------------------------
#
# Open device
png(filename = './plot1.png', width = 480, height = 480, units='px')
# Plot data
#with(txtfile, hist(Global_active_power, xlab = 'Global Active Power (kilowatts)', main = 'Global Active Power', col = 'red'))
hist(txtfile$Global_active_power, xlab = 'Global Active Power (kilowatts)', main = 'Global Active Power', col = 'red')
# Close device
dev.off()